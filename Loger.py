...
LOG_FORMAT = '[%(asctime)s.%(msecs)03d] %(filename)s -> %(funcName)s line:%(lineno)d [%(levelname)s] : %(message)s'  # 每条日志输出格式
console_format = '%(log_color)s[%(asctime)s.%(msecs)03d] %(filename)s -> %(funcName)s line:%(lineno)d [%(levelname)s] : %(message)s'
...
def get_logger(name=None):
    ...
    if LOG_ENABLED and LOG_TO_CONSOLE:
        ...
        formatter = colorlog.ColoredFormatter(fmt=console_format, log_colors=log_colors_config)
